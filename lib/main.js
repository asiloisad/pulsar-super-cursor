...
module.exports = {
  ...
  config: {
    cursorDecoration: {
      order: 1,
      title: 'Cursor decoration switch',
      description: 'The main cursor is marked with special CSS when the number of cursors is more than two',
      type: 'boolean',
      default: true,
    },
    decorationTime: {
      order: 2,
      title: 'Time until cursor decoration disappears',
      description: 'Set the time in ms after which the super-cursor decoration disappears. If the timer is set to zero, the decoration is persistant',
      type: 'integer',
      minimum: 0,
      default: 2000,
    },
  ...
  update(editor) {
    ...
    editor.consolidateSelections = () => {
      ...
      if (selections.length > 1) {
        ...
        for (let selection of selections) {
          if (inow!=ireq) { selection.destroy() }
          inow = inow+1
        }
    ...
    editor.cursorHighlight = () => {
      ...
      if (!this.cursorDecoration || editor.cursors.length<=1) { return }
      ...
      if (this.decorationTime) { setTimeout(() => {
        decoration.destroy() }, this.decorationTime) }
    }
  ...
  previous() {
    this.editor.cursorIndex = Math.max(0,
