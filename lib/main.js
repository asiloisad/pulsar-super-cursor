...
export default {
  ...
  getCursor() {
    return atom.workspace.getActiveTextEditor().getLastCursor()
  },

  getSelection() {
    return atom.workspace.getActiveTextEditor().getLastSelection()
  },
  ...
  toggle() {
    if (cursor = this.getCursor()) {
      ...
      for (let isel of editor.getSelections()) {
        if (isel==selection) {
          continue
      ...
      for (let i=0; i<lastRow; i++) {
        let marker = editor.selectionsMarkerLayer.markBufferPosition([i,0], { invalidate: 'never' })
        if (marker) {
          editor.getLastCursor().setBufferPosition(buffPos)
          return
        }
      }
    }
